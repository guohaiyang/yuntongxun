...
- (void)loadView
{
    ...
    {
        UIBarButtonItem *left = [[UIBarButtonItem alloc] initWithCustomView:[CommonTools navigationItemBtnInitWithTitle:@"返回" target:self action:@selector(backToView)]];
        self.navigationItem.leftBarButtonItem = left;
        [left release];
    }
...
-(void)networkStatistic
{
    ...
    if (networkStatistic)
    {
        ...
        netStatusLabel.text = [NSString stringWithFormat:@"移动网络发送：%0.2f（kB）移动网络接收：%0.2f（kB）；wifi网络发送：%0.2f（kB）wifi网络接收：%0.2f（kB）",networkStatistic.txBytes_sim / 1024.,networkStatistic.rxBytes_sim / 1024. ,networkStatistic.txBytes_wifi / 1024.,networkStatistic.rxBytes_wifi / 1024. ];
    }
}
