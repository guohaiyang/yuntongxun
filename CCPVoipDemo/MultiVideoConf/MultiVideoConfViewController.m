...
- (void)loadView
{
    ...
    MultiVideoView* _mainView = [[MultiVideoView alloc] initWithFrame:CGRectMake(11, 33, 195, 195)];
    ...
    MultiVideoView* _view1 = [[MultiVideoView alloc] initWithFrame:CGRectMake(mainView.frame.origin.x+mainView.frame.size.width+11, 33, 92, 92)];
    ...
    MultiVideoView* _view2 = [[MultiVideoView alloc] initWithFrame:CGRectMake(view1.frame.origin.x, view1.frame.origin.y+92+11, 92, 92)];
    ...
    MultiVideoView* _view3 = [[MultiVideoView alloc] initWithFrame:CGRectMake(mainView.frame.origin.x, mainView.frame.origin.y+195+11, 92, 92)];
    ...
    MultiVideoView* _view4 = [[MultiVideoView alloc] initWithFrame:CGRectMake(mainView.frame.origin.x+92+11, mainView.frame.origin.y+195+11, 92, 92)];
    ...
    MultiVideoView* _view5 = [[MultiVideoView alloc] initWithFrame:CGRectMake(mainView.frame.origin.x+92+11+92+11, mainView.frame.origin.y+195+11, 92, 92)];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    [self.modelEngineVoip setVideoView:nil andLocalView:view1.bgView];
    
//    [self.modelEngineVoip.VoipCallService setVideoConferenceAddr:@"42.121.115.160" andPort:7691];
...
- (void) setVideoConf:(NSString *)videoConf
{
    if (self.confAddr == nil && videoConf!=nil && videoConf.length > 0)
    {
        ...
        NSLog(@"\r\n////////////////////////\r\n%@\r\n//////////////////",self.confAddr);
        [self.modelEngineVoip.VoipCallService setVideoConferenceAddr:self.confAddr];
    }
}
...
{
    MultiVideoView* video = (MultiVideoView*)[self.bgView viewWithTag:index];
...
-(void)resetVideoViewFrame
{
    ...
    {
        MultiVideoView* tmpView = (MultiVideoView*)[self.bgView viewWithTag:i];
...
-(void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet.tag == 105)
    {
        if (buttonIndex == 0)
        {
            ...
            if (self.myVideoState == 0)
            {
                [self.modelEngineVoip.VoipCallService unpublishVideoInVideoConference:self.curVideoConfId ofAppId:self.modelEngineVoip.appID];
            }
            else
            {
                [self.modelEngineVoip.VoipCallService publishVideoInVideoConference:self.curVideoConfId ofAppId:self.modelEngineVoip.appID];
            }
        }
    ...
    else if (actionSheet.tag == 100)
    {
        ...
        if ([view2.strVoip isEqualToString:self.curMember])
        {
            view =self.view2;
        }
        else if ([view3.strVoip isEqualToString:self.curMember])
        {
            view = self.view3;
        }
        else if ([view4.strVoip isEqualToString:self.curMember])
        {
            view = self.view4;
        }
        else if ([view5.strVoip isEqualToString:self.curMember])
        {
            view = self.view5;
        }
        ...
        if (buttonIndex == 0)
        {
            NSLog(@"\r\n/////////////////////////////\r\nconfid=%@\r\nmember=%@///////////////////////",self.curVideoConfId,self.curMember);
            
            if (view.isDisplayVideo)
            {
                [self.modelEngineVoip.VoipCallService cancelConferenceMemberVideoWithAccount:self.curMember andVideoConference:self.curVideoConfId andPwd:nil];
                videoCount --;
            ...
            else
            {
                ...
                {
                    if ([self.curMember isEqualToString:member.number])
                    {
                        ...
                        if (addrarr.count == 2)
                        {
                            ...
                            [self.modelEngineVoip.VoipCallService requestConferenceMemberVideoWithAccount:self.curMember andDisplayView:view.bgView andVideoConference:self.curVideoConfId andPwd:nil andPort:port.integerValue];
                            videoCount ++;
                        }
                    }
                }
...
- (void)addMemberToView:(VideoConferenceMember*)member
{
    if (member && [member.number isEqualToString:self.modelEngineVoip.voipAccount])
    {
        self.view1.isDisplayVideo = YES;
        self.view1.strVoip = member.number;
        self.view1.voipLabel.text = [member.number substringFromIndex:[member.number length]-4];
    ...
    for (int i = VideoConfVIEW_View1+1; i<VideoConfVIEW_View1+5; i++)
    {
        ...
        if (tmpView && tmpView.strVoip == nil)
        {
            if (member)
            {
                NSLog(@"addMemberToView voip is %@ type is %@ screen is %d count is %d",member.number,member.type,member.screenType,self.membersArray.count);
                ...
                else
                {
                    ...
                    if (addrarr.count == 2)
                    {
                        ...
                        NSLog(@"\r\n/////////////////////////////\r\nconfid=%@\r\nmember=%@\r\n///////////////////////",self.curVideoConfId,member.number);
                        NSString* port = [addrarr objectAtIndex:1];
                        [self setVideoConf:[addrarr objectAtIndex:0]];
                        [self.modelEngineVoip.VoipCallService requestConferenceMemberVideoWithAccount:member.number andDisplayView:tmpView.bgView andVideoConference:self.curVideoConfId andPwd:nil andPort:port.integerValue];
...
-(void)removeMemberFromView:(VideoConferenceMember*)member
{
    for (int i = VideoConfVIEW_View1; i<VideoConfVIEW_View1+5; i++)
    {
        MultiVideoView* tmpView = (MultiVideoView*)[self.bgView viewWithTag:i];
        if (tmpView && tmpView.strVoip != nil)
        {
            if (member && [tmpView.strVoip isEqualToString:member.number])
            {
                videoCount --;
                [self.modelEngineVoip.VoipCallService cancelConferenceMemberVideoWithAccount:member.number andVideoConference:self.curVideoConfId andPwd:nil];
...
-(void)exitVideoConf
{
    [self.modelEngineVoip.VoipCallService exitVideoConference];
    [self backToView];
}
...
//        {
//            if ([self.modelEngineVoip.voipAccount isEqualToString:who])
//            {
//                [self displayProgressingView];
...
//        }
        ...
        if ([self.curVideoConfId isEqualToString:msg.conferenceId])
        {
            ...
            for (NSString *who in joinMsg.joinArr)
            {
                ...
                member.type = @"0";
                member.screenType = 0;
                member.videoState = joinMsg.videoState;
                member.videoSource = joinMsg.videoSrc;
    ...
    else if([msg isKindOfClass:[VideoConferencePublishVideoMsg class]])
    {
        if ([msg.conferenceId isEqualToString: self.curVideoConfId])
        {
            VideoConferencePublishVideoMsg* videoMsg = (VideoConferencePublishVideoMsg*)msg;
            for (VideoConferenceMember *member in self.membersArray)
            {
                if ([videoMsg.who isEqualToString:member.number])
                {
                    member.videoState = videoMsg.videoState;
                    member.videoSource = videoMsg.videoSrc;
                    break;
                }
            }
            NSLog(@"///////////////////\r\nVideoConferencePublishVideoMsg=%@\r\n///////////////////",videoMsg);
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示"
                                                            message:[NSString stringWithFormat:@"%@发布视频",videoMsg.who]
                                                           delegate:nil
                                                  cancelButtonTitle:@"确定"
                                                  otherButtonTitles: nil];
            [alert show];
    ...
    else if([msg isKindOfClass:[VideoConferenceUnpublishVideoMsg class]])
    {
        if ([msg.conferenceId isEqualToString: self.curVideoConfId])
        {
            VideoConferenceUnpublishVideoMsg* videoMsg = (VideoConferenceUnpublishVideoMsg*)msg;
            for (VideoConferenceMember *member in self.membersArray)
            {
                if ([videoMsg.who isEqualToString:member.number])
                {
            ...
            NSLog(@"//////////////////\r\nVideoConferenceUnpublishVideoMsg=%@\r\n///////////////////",videoMsg);
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示"
                                                            message:[NSString stringWithFormat:@"%@取消发布视频",videoMsg.who]
                                                           delegate:nil
                                                  cancelButtonTitle:@"确定"
                                                  otherButtonTitles: nil];
            [alert show];
...
- (void)onVideoConferenceMembersWithReason:(CloopenReason *) reason andData:(NSArray*)members
{
    ...
    if (reason.reason == 0)
    {
        ...
        for (VideoConferenceMember *member in tmpMemberarr)
        {
            if ([self.modelEngineVoip.voipAccount isEqualToString:member.number])
            {
                meInfo = member;
                self.myVideoState = member.videoState;
                [meInfo retain];
                [self.membersArray removeObject:member];
            }
            [self addMemberToView:member];
...
//            [self.modelEngineVoip queryMembersInVideoConference:curVideoConfId];
//        }
        [self performSelector:@selector(multivideoquermember) withObject:nil afterDelay:1.0];
    }
    ...
    {        
        statusView.text = [NSString stringWithFormat:@"第三方鉴权失败"];
        UIAlertView *alertview = [[UIAlertView alloc]  initWithTitle:@"提示" message:@"第三方鉴权失败"  delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        if (self.myAlertView)
        {
            [self.myAlertView dismissWithClickedButtonIndex:0 animated:NO];
        }
...
-(void)createMultiVideoWithAutoClose:(BOOL) isAutoClose andIsPresenter:(BOOL) isPresenter andiVoiceMod:(NSInteger)iVoiceMod andAutoDelete:(BOOL)bAutoDelete andIsAutoJoin:(BOOL) isAutoJoin
{
    [self displayProgressingView];
    [self.modelEngineVoip setUIDelegate:self];
    [self.modelEngineVoip.VoipCallService startMultiVideoConferenceInAppId:self.modelEngineVoip.appID withName:Confname andSquare:5 andKeywords:@"" andPassword:@"" andIsAutoClose:isAutoClose andIsPresenter:isPresenter andVoiceMod:1 andAutoDelete:bAutoDelete andIsAutoJoin:isAutoJoin];
}
...
-(void)joinInVideoConf
{
    //[self displayProgressingView];
    [self.modelEngineVoip setUIDelegate:self];
    [self.modelEngineVoip.VoipCallService joinInVideoConference:self.curVideoConfId];
}
...
- (void)onCallVideoRatioChanged:(NSString *)callid andVoIP:(NSString *)voip andIsConfrence:(BOOL)isConference andWidth:(NSInteger)width andHeight:(NSInteger)height
{
    if (isConference && voip.length > 0) {
        ...
        {
            MultiVideoView* tmpView = (MultiVideoView*)[self.bgView viewWithTag:i];
            if (tmpView.strVoip.length > 0 && [tmpView.strVoip isEqualToString:voip])
            {
