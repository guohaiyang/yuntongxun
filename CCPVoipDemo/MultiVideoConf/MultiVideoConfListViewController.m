...
#import "MultiHighlightDelegateCell.h"
@interface MultiVideoConfListViewController ()
{
    UITableView *ConfListView;
    NSMutableArray *MultiVideoConfsArray;
    UIView * noneView;
}
@end
...
- (void)loadView
{
    ...
    MultiVideoConfsArray = self.modelEngineVoip.multiVideoconferenceListArray;
...
- (void)viewRefresh
{
    if (MultiVideoConfsArray.count > 0)
    {
...
{
    return [MultiVideoConfsArray count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        cell = [[[MultiHighlightDelegateCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:cellid] autorelease];
    ...
    VideoConference *ConfInfo = [MultiVideoConfsArray objectAtIndex:indexPath.row];
...
{
    VideoConference *selectConf = [MultiVideoConfsArray objectAtIndex:indexPath.row];
    if(selectConf.conferenceId.length > 0  && selectConf.square > selectConf.joinNum)
    {
        ...
        for (VideoConference *conf in MultiVideoConfsArray)
        {
        ...
        if (iscreator)
        {
            ...
            self.curRoomVideoAddr = selectConf.confAttrs;
        ...
        {
            [self joinConfWithRoomNo:selectConf.conferenceId andRoomname:name andCreator:iscreator andAddr:selectConf.confAttrs];
        }
    }
