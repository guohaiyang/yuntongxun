...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    UITextField* tf = (UITextField*)[self.view viewWithTag:802];
    tf.text = @"";
}
...
- (void)updateBtnDisplay
{
    ...
    {
        [self performSelectorOnMainThread:@selector(refreshTimes) withObject:self waitUntilDone:YES];
    }
}
-(void)refreshTimes
{
    btnGetVerifyCode.titleLabel.text = [NSString stringWithFormat:@"%d秒后重新获取",remainTime];
}
@end
