...
- (void)onDisconnected
{
    if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(onLogout)])
    {
        [self.UIDelegate onLogout];
    }
}
...
-(void)parseIMMsg:(InstanceMsg*) msg
{
...
- (void)onReceiveInstanceMessage:(InstanceMsg*) msg
{
    ...
    if ([msg isKindOfClass:[IMTextMsg class]]||[msg isKindOfClass:[IMAttachedMsg class]])
    {
        [self parseIMMsg:msg];
    }
