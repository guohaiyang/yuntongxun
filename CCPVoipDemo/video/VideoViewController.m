...
- (void)updateRealtimeLabel
{
    ...
    if(ssInt > 0 && ssInt % 4 == 0 )
    {
        ...
        if (networkStatistic)
        {
            self.netStatusLabel.text = [NSString stringWithFormat:@"移动网络发送：%0.2f（kB）移动网络接收：%0.2f（kB）；wifi网络发送：%0.2f（kB）wifi网络接收：%0.2f（kB）",networkStatistic.txBytes_sim / 1024.,networkStatistic.rxBytes_sim / 1024. ,networkStatistic.txBytes_wifi / 1024.,networkStatistic.rxBytes_wifi / 1024. ];
        }
    }
    if (hhInt > 0) {
...
-(void)callingViewLayout
{
    ...
    if (self.callingView == nil)
    {
        ...
        UILabel *tmplabel = [[UILabel alloc] initWithFrame:CGRectMake(5.0f, 40.0f, 300.0f, 18.0f)];
        tmplabel.backgroundColor =  [UIColor clearColor];
        self.netStatusLabel = tmplabel;
        [self.callingView addSubview:self.netStatusLabel];
        tmplabel.textColor = [UIColor whiteColor];
        netStatusLabel.numberOfLines = 0;
        netStatusLabel.lineBreakMode = 0;
        tmplabel.font = [UIFont systemFontOfSize:8];
        [tmplabel release];
...
-(void)switchMediaType:(id)sender
{
    ...
    [self.modelEngineVoip.VoipCallService requestSwitchCall:self.callID toMediaType:type];
}
